...
<section class="bg-white dark:bg-gray-900">
    <form class="grid grid-cols-2 gap-4" enctype="application/x-www-form-urlencoded" method="post">
        @csrf
        <div class="px-6">
            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="hdd_type">
                HDD Type
            </label>
            <div class="relative">
...
<section>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400" id="servertable">
...
<script type="application/javascript">
    ...
    $('#filter').on('click', function (event) {
        ...
        const capacityMin = $('#min').text();
        const capacityMax = $('#max').text();
                ...
                if (data.length > 0) {
                    $.each(data, function (key, value) {
                        const html = '<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">' +
                            '<th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">' + value.id + '</th>' +
                            '<td class="px-6 py-4">' + value.model + '</td>' +
                            '<td class="px-6 py-4">' + value.hdd + '</td>' +
                            '<td class="px-6 py-4">' + value.ram + '</td>' +
                            '<td class="px-6 py-4">' + value.location + '</td>' +
                            '<td class="px-6 py-4">' + value.price + '</td>' +
                            '</tr>';

                        $('#serverTableBody').append(html);
                    });
                } else {
                    $('#serverTableBody').append('No matching servers found');
                }
